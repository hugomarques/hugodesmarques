...
Recentemente, diversas APIs e *libs* estão surgindo para minimizar o esforço de escrever código Java e tornar a linguagem mais produtiva para o desenvolvedor; Lombok é uma delas.
...
Como você já deve suspeitar, se usar o método acima passando *name* ou *creatureType* com valor *null*, ele irá lançar uma *NullPointerException*. 
 ...
 * Outras opções também são possíveis, como exclusão de atributos.
*/
@EqualsAndHashCode
public class Creature {
	String name;
...
{%highlight java linenos %}
/* Mais conhecida como "adeus java bean verborrágico".
 * A anotação abaixo é o mesmo que anotar sua classe com @Getter @Setter @EqualsAndHashCode @ToString
 *
*/
@Data
public class Creature {
...
public class CreatureResource {
	...
	* A anotação cria uma variável similar à seguinte linha de código: 
